...
def open_new_book_window():
    ...
    def add_new():
        ...
        try: 
            ...
            mycursor.execute("SELECT Imie, Nazwisko FROM Student WHERE Imie == (?) AND Nazwisko == (?)",(new_imie,new_nazwisko))
            if mycursor.fetchall() == []:
                mycursor.execute("INSERT INTO Student (Imie,Nazwisko) VALUES (?,?)",(new_imie,new_nazwisko))
            mycursor.execute("SELECT IdStudent FROM Student WHERE Imie == (?) AND Nazwisko == (?)",(new_imie,new_nazwisko))
            ...
            mycursor.execute("INSERT INTO Ocena (IdStudent,IdPrzedmiot,Ocena) VALUES (?,?,?)",(IdStudent,IdPrzedmiot,new_ocena))
            mydb.commit()
    ...
    if selected_item:
        ...
        def update_data():
            try:
                ...
                mycursor.execute("SELECT p.IdPrzedmiot FROM Przedmiot AS p JOIN Ocena AS o ON p.IdPrzedmiot=o.IdPrzedmiot WHERE o.IdOcena = (?)",(id_entry.get(),))
                result = [mycursor.fetchall()[0]][0][0]
                if result != []:
                    mycursor.execute("UPDATE Przedmiot SET Nazwa = (?) WHERE IdPrzedmiot = (?)",(przedmiot_entry.get(),result))

                mycursor.execute("SELECT s.IdStudent FROM Student AS s JOIN Ocena AS o ON s.IdStudent=o.IdStudent WHERE o.IdOcena = (?)",(id_entry.get(),))
                result = [mycursor.fetchall()[0]][0][0]
                if result != []:
                    mycursor.execute("UPDATE Student SET Imie = (?), Nazwisko = (?) WHERE IdStudent = (?)",(imie_entry.get(),nazwisko_entry.get(),result))
